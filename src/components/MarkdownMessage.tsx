    ...
    <div className={`prose ${isAssistant ? 'prose-invert' : ''} max-w-none`}>
      <ReactMarkdown
        ...
        components={{
          ...
          p: ({ children }) => (
            <p className={`${isAssistant ? 'text-white/90' : 'text-gray-900'} leading-relaxed font-body text-[15px] mb-3`}>
              {children}
            </p>
          ),
          h1: ({ children }) => (
            <h1 className={`${isAssistant ? 'text-white' : 'text-gray-900'} text-xl font-heading font-semibold mb-3 mt-4`}>
              {children}
            </h1>
          ),
          h2: ({ children }) => (
            <h2 className={`${isAssistant ? 'text-white' : 'text-gray-900'} text-lg font-heading font-semibold mb-2 mt-3`}>
              {children}
            </h2>
          ),
          h3: ({ children }) => (
            <h3 className={`${isAssistant ? 'text-white' : 'text-gray-900'} text-base font-heading font-semibold mb-2 mt-3`}>
              {children}
            </h3>
          ),
          strong: ({ children }) => (
            <strong className={`${isAssistant ? 'text-white' : 'text-gray-900'} font-semibold`}>
              {children}
            </strong>
          ),
          em: ({ children }) => (
            <em className={`${isAssistant ? 'text-white/80' : 'text-gray-700'} italic`}>
              {children}
            </em>
          ),
          ul: ({ children }) => (
            <ul className={`${isAssistant ? 'text-white/90' : 'text-gray-900'} list-disc pl-6 mb-3 space-y-1`}>
              {children}
            </ul>
          ),
          ol: ({ children }) => (
            <ol className={`${isAssistant ? 'text-white/90' : 'text-gray-900'} list-decimal pl-6 mb-3 space-y-1`}>
              {children}
            </ol>
          ),
          li: ({ children }) => (
            <li className={`${isAssistant ? 'text-white/90' : 'text-gray-900'} leading-relaxed font-body text-[15px]`}>
              {children}
            </li>
          ),
          blockquote: ({ children }) => (
